...
function ProfileDropdown({ navigate, dispatch }) {
  ...
  return (
    <div
      // ref={buttonRef}
      className="flex items-center gap-x-1 md:gap-x-4 text-white"
    >
      {/* dashboard */}
      <div className="hidden md:block md:gap-x-2">
        <Link
          className="px-4 rounded-lg shadow-sm shadow-blue-100 py-[8px] bg-richblack-800 hidden md:block"
      ...
      <button
        ...
        onClick={() => {
          setConfirmationModal({
            ...
            btn2Text: "Cancel",
            btn1Handler: () => {
              logout(navigate, dispatch), setConfirmationModal(null);
            },
      ...
      {profileDropDownModal && (
        <ProfileDropDownConModal
          modalData={profileDropDownModal}
          setModalData={setProfileDropDownModal}
        />
      )}
    </div>
  );
